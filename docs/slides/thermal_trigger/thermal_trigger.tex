...
\begin{columns}[c]
\column{0.62\textwidth}
\includegraphics[width=\textwidth]{sn_optimization.pdf}
\column{0.38\textwidth}\small
The correlated capture background falls \emph{faster} than the signal
(capture-$\gamma$ Compton edges), so both S/N and the significance
FOM $\varepsilon/\sqrt{B}$ \textbf{rise with threshold} through the
reliable region.\\[6pt]
Above $\sim$3.5\,MeV the background is MC-limited ($<10$ counts) --- we can't
measure it, and signal is already halved.
\end{columns}
\vfill
\keybox{Best measurable operating point: \textbf{plastic $b\approx$ 2.6--3.0\,MeV
(0.6--0.7\,MIP)}. Correlated background $<0.1$/pulse, $\sim$50\% of X17 \& IPC
retained, IPC/X17 undistorted, $\sim$120--140 legs/pulse (under the 200 budget).}
\end{frame}
...
\vfill
\begin{itemize}\small
\item Single-leg (L1) rate is the DAQ constraint --- the \emph{pair}
      coincidence is $<2$/pulse. If you can form the 2-arm coincidence in
      hardware, $b$ can go as low as you like.
\item Sim gives $\sim$450--550 legs/pulse at the floor, $\sim2\times$ below the
      measured $\sim$1000 --- the balance is non-capture background (room
      return, flash afterglow) \emph{not} in the sim; identify it before
      trusting the rate axis absolutely.
\end{itemize}
\end{frame}
