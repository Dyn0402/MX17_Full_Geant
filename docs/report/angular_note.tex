...
given the capsule multiple scattering, smeared by the smallest feasible
resolution (the target-centre chord, $\sig{} = 15^{\circ}$): a
$\sim$40$^{\circ}$-FWHM bump at a quarter of the height. \textbf{Middle:}
that truth signal stacked on the recorded IPC continuum --- even unsmeared
it is only a $\sim$60\% local excess. \textbf{Right:} both species smeared
--- \emph{this is the spectrum the analysis actually has to extract the
X17 signal from}: a $\lesssim$17\% broad enhancement of a featureless
falling continuum. And
...
Figure~\ref{fig:ipcdil} and the money plot in Part~I put the two species on
one axis. Three observations. \emph{(i)} The IPC continuum is nearly
invariant under the resolution; the entire information loss is the signal
...
$S \approx 135$ on $B \approx 950$ ($S/\sqrt{B} \approx 4.4$ per 30 days,
...
\item \textbf{Gaussian illustration.} The money/dilution plots smear with a
  Gaussian of the measured $\sig{}$ + bias; the real response has heavier
  tails (RMS/$\sig{} \approx 1.25$), which slightly worsens the realistic
  picture. The template-fit study must use the full response matrix, not the
  Gaussian.
