...
included: \textbf{27.8\% for X17, 27.4\% for IPC} --- nose-first, 2026-07-26
[\texttt{analysis/pairs\_nose}]; were 19.6/23.6\% in the pre-flip geometry).
That gives $\sim$9.1 recorded X17/day, $\sim$270 per 30-day run, on a
recorded signal-to-IPC ratio of $\approx$0.026 (the upstream nose-first pair
vertices raise the two-arm acceptance, and X17 more than IPC). The remaining question is whether those events form a
...
\item \textbf{Fit the spectrum, don't cut it.} The response matrix
  ...
  expected significance of that fit ($\sim$270 signal events per 30 days at
  the conservative $\alpha_{\mathrm{IPC}}=2.1\times10^{-3}$, nose-first
  acceptance). This has now been computed for the standard July/LS3 windows at
  $\alpha_{\mathrm{IPC}}=3.5\times10^{-3}$: Asimov $Z\approx3.5\sigma$ (July
  0.2--0.7\,MeV) / $6.4\sigma$ (LS3 0.2--2\,MeV), stat-only, up $\times1.31$
  from the pre-flip 2.6/4.9$\sigma$ on the higher MM-double acceptance --- with
  the IPC-shape systematic still setting the achievable CL
  (\texttt{scripts/make\_slides\_figures.py}).
...
\item \textbf{Yield inputs.} Production 32.4 X17/day ($E_n \gtrsim
  20$\,keV, MeV-note decades) and MM-double acceptances 27.8\%/27.4\% (nose-first; were 19.6/23.6\%)
